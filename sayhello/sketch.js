...
function draw() {
  ...
  charOpacity = charOpacities[0];
  let imageIndex = floor(map(charOpacity, 0, 255, 0, imagesCount-1));
  console.log(imageIndex);
  image(images[imageIndex], 0, 0, 512, 512);
  // }

  // if (frameCount % 2 == 0){
  //   if (charOpacities[0] > 250 || charOpacities[0] < 5){
  //     change = -change;
  //   }
  //   charOpacities[0] += change;
  //   // console.log(charOpacities[0]);
  // }
...
function lightUpLetter(letter) {
  switch (letter) {
    ...
    case "R":
      ...
      break;
    case "Nothing":
      if (currentCharIndex == 0 && charOpacities[0] > 0) {
        charOpacities[0] -= 5;
        console.log("grow old")
      }
      break;
  }
}